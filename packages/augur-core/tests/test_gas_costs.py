...
def test_disputeWindowCreation(localFixture, universe, cash):
    endTime = localFixture.chain.head_state.timestamp + timedelta(days=365).total_seconds()
...
def test_marketCreation(localFixture, universe):
    ...
    endTime = localFixture.chain.head_state.timestamp + timedelta(days=1).total_seconds()
